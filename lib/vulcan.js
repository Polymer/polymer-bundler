...
Vulcan.prototype = {
  ...
  collectLicenseComments: function(nodes, collection) {
    for (var i = 0, n; i < nodes.length; i++) {
      n = nodes[i];
      if (this.isLicenseComment(n)) {
        collection.push(n);
      }
    }
  },
  ...
  flatten: function flatten(tree, bodyFragment, mainDocUrl) {
    ...
    if (imports) {
      ...
      for (var i = 0, im, thisImport; i < imports.length; i++) {
        ...
        this.collectLicenseComments(importDoc.childNodes, licenseComments);
        var importHtml = importHead.parentNode;
        // check child nodes of <html>
        this.collectLicenseComments(importHtml.childNodes, licenseComments);
