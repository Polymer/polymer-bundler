...
function buildLoader(abspath, excludes, bowerDir) {
  var loader = new hyd.Loader();
  var fsOptions = {};
  if (abspath) {
    fsOptions.root = path.resolve(abspath);
    fsOptions.redirect = bowerDir;
  }
...
var Vulcan = function Vulcan(opts) {
  ...
  this.bowerDir = opts.bowerDir ? opts.bowerDir : 'bower_components';
  ...
  if (!opts.loader) {
    this.loader = buildLoader(this.abspath, this.excludes, this.bowerDir);
  } else {
...
Vulcan.prototype = {
  ...
  getImplicitExcludes: function getImplicitExcludes(excludes) {
    ...
    var loader = buildLoader(this.abspath, null, this.bowerDir);
  ...
  _process: function _process(target, cb) {
    ...
    chain = chain.then(function(){
      ...
      for (var i = 0; i < this.addedImports.length; i++) {
        ...
        dom5.insertBefore(head, head.childNodes[0], newImport);
      }
