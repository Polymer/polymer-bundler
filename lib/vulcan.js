...
Vulcan.prototype = {
  ...
  flatten: function flatten(tree, rootDocPath) {
    var isMainDoc = tree.href === rootDocPath;
    ...
    if (imports) {
      for (var i = 0, im, thisImport; i < imports.length; i++) {
        ...
        var importDoc = this.flatten(im, rootDocPath);
        // rewrite urls
  ...
  _process: function _process(target, cb) {
    ...
    chain = chain.then(function(){
      ...
      var flatDoc = this.flatten(tree, tree.href);
