...
var CommentMap = require('./commentmap.js');
...
function compressJS(content, inline) {
  var keepComment = function(node, token) {
    var d = token.value;
    var keep = (/@license/.test(d));
    if (keep && (keep = !comments.has(d))) {
      comments.set(d);
  ...
  try {
    ...
    return ast.print_to_string({inline_script: inline, comments: keepComment});
  } catch (e) {
...
function checkNewPolymer($, filename) {
  var matches = searchAll($, constants.DOM_MODULE);
  if (matches.length) {
    throw new Error(constants.NEW_POLYMER_ERROR + ' File: ' + filename);
  }
}
...
function handleMainDocument() {
  ...
  var charsetMeta = searchAll($, 'meta[charset]');
  if (charsetMeta.length === 0) {
    $('head').append(constants.CHARSET_META);
  }
