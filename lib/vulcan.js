...
function buildLoader(abspath, excludes) {
  ...
  if (excludes) {
    ...
    excludes.forEach(function(r) {
      // terrible hack for working around windows paths
      // TODO(dfreedm): make a more general path-to-url converter in hydrolysis
      var u = url.format(url.parse(path.resolve(r)));
      loader.addResolver(new hyd.NoopResolver(u));
    });
  }
  return loader;
}
...
var Vulcan = function Vulcan(opts) {
  var strip = opts.implicitStrip;
  // implicitStrip should be true by default
  if (strip === undefined) {
    strip = true;
  }
  this.implicitStrip = strip;
  this.abspath = opts.abspath && path.resolve(opts.abspath);
  this.pathResolver = new PathResolver(this.abspath);
  this.excludes = opts.excludes;
  this.stripExcludes = opts.stripExcludes;
  this.stripComments = opts.stripComments;
  this.enableCssInlining = opts.inlineCss;
  this.enableScriptInlining = opts.inlineScripts;
  if (!opts.loader) {
...
Vulcan.prototype = {
  ...
  getImplicitExcludes: function getImplicitExcludes(excludes) {
    ...
    excludes.forEach(function(exclude) {
      analyzedExcludes.push(analyzer._getDependencies(exclude));
    });
  ...
  _process: function _process(target, cb) {
    ...
    if (this.enableScriptInlining) {
      chain = chain.then(function(doc) {
        return this.inlineScripts(doc, target);
      }.bind(this));
    }
    if (this.enableCssInlining) {
      chain = chain.then(function(doc) {
        return this.inlineCss(doc, target);
      }.bind(this));
    }
