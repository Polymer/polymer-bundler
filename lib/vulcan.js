...
Vulcan.prototype = {
  ...
  flatten: function flatten(tree, isMainDoc, rootDocPath) {
    if (!rootDocPath) {
      rootDocPath = tree.href
    }
    ...
    if (imports) {
      for (var i = 0, im, thisImport; i < imports.length; i++) {
        ...
        var importDoc = this.flatten(im, false, rootDocPath);
        // rewrite urls
        this.pathResolver.resolvePaths(importDoc, im.href, tree.href, rootDocPath);
    ...
    //Remove temporary <attr>-resolved attributes from nodes when ready
    if (isMainDoc) {
      this.pathResolver.removeResolvedFlagAttributes(doc);
    }
    return doc;
  },
