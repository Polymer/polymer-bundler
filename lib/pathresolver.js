...
function resolvePaths($, input, output, webAbsPath) {
      ...
      if (val) {
        if (val.search(constants.URL_TEMPLATE) < 0) {
          if (a === 'style') {
            ...
            el.attr(a, rewriteRelPath(input, output, val, webAbsPath));
          }
        }
      }
    });
...
function rewriteRelPath(inputPath, outputPath, rel, webAbsPath) {
  if (constants.ABS_URL.test(rel)) {
    // if(webAbsPath!=null  && !constants.REMOTE_ABS_URL.test(rel)){
    //   var abs = path.resolve(inputPath, path.join(webAbsPath, rel));
    //   rel = path.relative(outputPath, abs);
    // }
    return rel;
  } else if (webAbsPath != null) {
    var abs = path.resolve(inputPath, rel);
    var absBase = new RegExp('^' + RegExp.escape(webAbsPath));
    abs = abs.replace(absBase, '');
    return abs;
  } else {
    var abs = path.resolve(inputPath, rel);
