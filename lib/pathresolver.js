...
PathResolver.prototype = {
  ...
  rewriteRelPath: function rewriteRelPath(importUrl, mainDocUrl, relUrl) {
    ...
    if (parsedFrom.protocol === parsedTo.protocol && parsedFrom.host === parsedTo.host) {
      var parsedFromDir = pathPosix.dirname(parsedFrom.pathname);
      // top-level file paths like `index.html` will return the dirname `.`.
      // To calculate the relative path below, we should resolve against the
      // root (`/`) instead.
      if (parsedFromDir === '.') {
        parsedFromDir = path.sep;
      }
      var pathname = pathPosix.relative(parsedFromDir, parsedTo.pathname);
      return url.format({
        pathname: pathname,
