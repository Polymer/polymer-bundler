...
## Unreleased
- [BREAKING] Upgraded to use `polymer-analyzer` version `3.0.0-pre.7` which requires the `ResolvedUrl` type in nearly all places where URLs are exchanged.  Eliminated the local `UrlString` type.
